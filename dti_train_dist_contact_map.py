...
def train(model, prot_model, optimizer, loss_fn, train_loader, val_loader, target_scaler, 
    ...
    val_losses = []
    val_pccs = []
    # scheduler after 1300 epochs
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=1300, eta_min=0.0001)
    for epoch in range(1, epochs+1):
        ...
        for i, batch in enumerate(train_loader):
            ...
            loss.backward()
            optimizer.step()
            # Scheduler step after 1300 epochs
            scheduler.step()
...
if __name__ == '__main__':
    ...
    models = ["SeqVec", "DistilProtBERT", "ProtBERT", "ESM2", "CNN"]
    ...
    distances_path = "data/kiba/pconsc4/"
    ...
    def extract_protein_contact_map(opts, contact_path):
        pconsc4_files = set(f for f in os.listdir(contact_path))
        all_distances = []
        for opt in opts:
            distances = []
            for index, row in tqdm(opt.iterrows(), total=len(opt)):
                protein = row["Target_ID"]
                if protein + ".npy" not in pconsc4_files:
                    contact = np.zeros((MAX_PROTEIN_LEN, MAX_PROTEIN_LEN))
                    distances.append(contact.flatten())
                    continue
                contact_map = np.load(contact_path + protein + ".npy") > 0.5
                # Convert contact_map to MAX_PROTEIN_LEN * MAX_PROTEIN_LEN size with last elements being removed if size is greater than MAX_PROTEIN_LEN
                if contact_map.shape[0] > MAX_PROTEIN_LEN:
                    contact_map = contact_map[:MAX_PROTEIN_LEN, :MAX_PROTEIN_LEN]
                else:
                    contact_map = np.pad(contact_map, ((0, MAX_PROTEIN_LEN - contact_map.shape[0]), (0, MAX_PROTEIN_LEN - contact_map.shape[1])), 'constant', constant_values=0)
                distances.append(contact_map.flatten())
            all_distances.append(distances)
        return all_distances

    # Load the train distances
    train_dist, valid_dist, test_dist = extract_protein_contact_map([train, val, test], distances_path)
